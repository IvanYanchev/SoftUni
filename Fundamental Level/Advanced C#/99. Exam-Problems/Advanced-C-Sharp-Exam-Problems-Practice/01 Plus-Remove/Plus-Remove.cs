    ...
    class PlusRemove
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                boardOfChars.Add(inputString.PadRight(100).ToList());
            }
            ...
            for (int row = 1; row < boardHeigth - 1; row++)
            {
                for (int col = 1; col < boardWidth - 1; col++)
                {
                    ...
                    {
                        if (char.ToLower(boardOfChars[row][col]) == char.ToLower(boardOfChars[row][col + 1]) &&
                            char.ToLower(boardOfChars[row][col]) == char.ToLower(boardOfChars[row + 1][col]) &&
                            char.ToLower(boardOfChars[row][col]) == char.ToLower(boardOfChars[row][col - 1]) &&
                            char.ToLower(boardOfChars[row][col]) == char.ToLower(boardOfChars[row - 1][col]))
                        {
